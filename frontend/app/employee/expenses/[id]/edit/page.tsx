...
export default function EditExpensePage() {
  ...
  const fetchExpense = async (expenseId: string) => {
    try {
      ...
      if (response.success) {
        ...
        setValue('categoryId', expenseData.category.id);
        setValue('description', expenseData.description);
        setValue('expenseDate', expenseData.expenseDate);
  ...
  const onSubmit = async (data: ExpenseFormData) => {
    try {
      setLoading(true);
      const response = await apiClient.updateExpense(expense.id, data);
      
      if (response.success) {
        router.push(`/employee/expenses/${expense.id}`);
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        <Link href={`/employee/expenses/${expense.id}`} className="p-2 text-gray-400 hover:text-gray-600">
          <ArrowLeft className="h-5 w-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Expense</h1>
          <p className="mt-1 text-sm text-gray-500">
            Update your expense claim
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="card-body">
                <dl className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <dd className="text-sm font-medium text-gray-900">Pending approval</dd>
                  </div>
                </dl>
              </div>
            </div>
        ...
        <div className="flex justify-end space-x-3">
          <Link href={`/employee/expenses/${expense.id}`} className="btn btn-outline">
            Cancel
          </Link>
          <button
            type="submit"
